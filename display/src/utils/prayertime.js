...
var prayerTimeCalculator = new PrayTimes("ISNA");

function setPrayerTimeParams() {
  prayerTimeCalculator.setMethod("MWL");
  ...
  // console.log(prayerTime.getTimes([2022, 5, 25], [-7.25, 112.75], 7));
}

function hmStrToRaw(hm) {
...
function adjustTimes(time, adjust) {
  let arr = [];
  for (let i = 0; i < 8; i++) {
    arr.push(adjustTime(time[i], adjust[i]));
  }
  return arr;
}

function getPrayerTimes(datetime, location, koreksi) {
  let prayerTimes = prayerTimeCalculator.getTimes(
    datetime,
    [location.lintang, location.bujur],
    location.timezone
  );
  ...
  let arrPrayerTimes = [
    hmStrToRaw(prayerTimes.imsak),
    hmStrToRaw(prayerTimes.fajr),
    hmStrToRaw(prayerTimes.sunrise),
    hmStrToRaw(prayerTimes.sunrise),
    hmStrToRaw(prayerTimes.dhuhr),
    hmStrToRaw(prayerTimes.asr),
    hmStrToRaw(prayerTimes.maghrib),
    hmStrToRaw(prayerTimes.isha),
  ];

  // console.log(arrPrayerTimes);

  arrPrayerTimes = adjustTimes(arrPrayerTimes, koreksi);

  // console.log(arrPrayerTimes);

  return arrPrayerTimes;
}

// function getAuqot(time, location) {
//   const prayerTimes = getPrayerTimes(time, location);
//   const auqot = [
//     { label: "Imsak", h: 3, m: 37, s: 0 },
//     { label: "subuh", h: 3, m: 57, s: 0 },
//     { label: "Syuruq", h: 5, m: 30, s: 0 },
//     { label: "Dhuha", h: 6, m: 15, s: 0 },
//     { label: "Dzuhur", h: 11, m: 50, s: 0 },
//     { label: "Ashar", h: 15, m: 1, s: 0 },
//     { label: "Maghrib", h: 17, m: 39, s: 0 },
//     { label: "Isya", h: 19, m: 0, s: 0 },
//   ];
//   // console.log(prayerTimes);
//   for (i = 0; i < 8; i++) {
//     auqot[i].h = prayerTimes[i].hour;
//     auqot[i].m = prayerTimes[i].minute;
//     auqot[i].s = prayerTimes[i].second;
//   }
//   return auqot;
// }

module.exports = { setPrayerTimeParams, getPrayerTimes };
...
//     strData: prayerTimes,
//     rawData: {
//       imsak: hmStrToRaw(prayerTimes.imsak),
//       fajr: hmStrToRaw(prayerTimes.fajr),
//       sunrise: hmStrToRaw(prayerTimes.sunrise),
//       dhuhr: hmStrToRaw(prayerTimes.dhuhr),
//       asr: hmStrToRaw(prayerTimes.asr),
//       sunset: hmStrToRaw(prayerTimes.sunset),
//       maghrib: hmStrToRaw(prayerTimes.maghrib),
//       isha: hmStrToRaw(prayerTimes.isha),
//     },
//   };
